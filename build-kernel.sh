...
function variables() {
    ...
    function essential() {
        AK_DIR=android/AnyKernel3
        TOOLCHAIN_DIR=android/aarch64-linux-android-4.9
        TOOLCHAIN_DIR_PREFIX=aarch64-linux-android-
        KERNEL_DIR=android/septickernel
        KERNEL_OUTPUT_DIR=android/septickernelout
        KERNEL_DEFCONFIG=rosy_defconfig
        KERNEL_NAME=Septic
    }

    function remote() {
        AK_REPO=https://github.com/royalturd/AnyKernel3-1.git
        AK_BRANCH=master
    ...
    function optional() {
        ...
        function ak_zip_attributes() {
            ...
            KERNEL_VERSION=1.0
            KERNEL_ANDROID_BASE_VERSION_IN_NAME=PIE

            CUSTOM_ZIP_NAME=
        }

        function toolchain() {
            TOOLCHAIN_NAME=GCC-4.9
            CLANG_NAME=
        }

        function kernel() {
            KERNEL_BUILD_USER=royalturd
            KERNEL_BUILD_HOST=ubuntu
        }
    ...
    function script() {
        ...
        REPORT_COMP_TIME_IN_MIN_AND_SEC=1
    }
